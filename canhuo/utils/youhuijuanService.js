...
const initInvolvedCouponArrayData = () =>{
  ...
  return initDataFromUrl('user/participate-list', params);
}
//我的优惠券包
...
const initDataFromUrl = async (url, params)=> {
...
function mapData(item) {
  ...
  return {
    ...
    accessType: initAccessType(item),
  }
}
//拼接代金券描述
function concatContent(item) {
  let content = "";
  switch (item.type) {
    case "1":
      //通用劵展示优惠金额
